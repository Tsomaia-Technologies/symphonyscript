...
export class MelodyNoteCursor extends NoteCursor<MelodyBuilder> {
    ...
    natural(): this {
        if (this.pendingOp.kind === 'note') {
            ...
            this.pendingOp.note = stripped as import('../../types/primitives').NoteName
        }
        return this
    }
    ...
    sharp(): this {
        if (this.pendingOp.kind === 'note') {
            ...
            if (match) {
                this.pendingOp.note = `${match[1]}#${match[3]}` as import('../../types/primitives').NoteName
            }
        }
        return this
    }
    ...
    flat(): this {
        if (this.pendingOp.kind === 'note') {
            ...
            const match = note.match(/^([A-Ga-g])([#b]?)(\d+)$/)
            if (match) {
                this.pendingOp.note = `${match[1]}b${match[3]}` as import('../../types/primitives').NoteName
            }
        }
        return this
    }
