// =============================================================================
// SynapticClipBuilder Tests - Phase 2 Composer Package
// =============================================================================

import { SynapticClipBuilder, parsePitch } from '../SynapticClipBuilder'
...
// =============================================================================
// SynapticClipBuilder Tests
// =============================================================================

describe('SynapticClipBuilder - Construction', () => {
    test('constructs with SiliconBridge', () => {
        const bridge = createTestBridge()
        const clip = new SynapticClipBuilder(bridge)

        expect(clip).toBeInstanceOf(SynapticClipBuilder)
        expect(clip.getCurrentTick()).toBe(0)
    })
})

describe('SynapticClipBuilder - Fluent Chaining', () => {
    ...
    test('play() returns this for chaining', () => {
        const bridge = createTestBridge()
        const clip1 = new SynapticClipBuilder(bridge)
        const clip2 = new SynapticClipBuilder(bridge)
...
describe('SynapticClipBuilder - Tick Advancement', () => {
...
describe('SynapticClipBuilder - Note Input', () => {
    test('accepts MIDI numbers', () => {
        ...
        expect(clip.getBuilder().getEntryId()).toBeDefined()
    })
...
describe('SynapticClipBuilder - Play/Synapse Connection', () => {
    test('play() creates synapse connection', () => {
        ...
        const exitId1 = clip1.getBuilder().getExitId()
        const entryId2 = clip2.getBuilder().getEntryId()
    ...
    test('chained play() creates multiple connections', () => {
        ...
        const clip3 = new SynapticClipBuilder(bridge)
        clip3.note('G4')

        clip1.play(clip2).play(clip3)

        expect(synapseExists(bridge, clip1.getBuilder().getExitId(), clip2.getBuilder().getEntryId())).toBe(true)
        expect(synapseExists(bridge, clip1.getBuilder().getExitId(), clip3.getBuilder().getEntryId())).toBe(true)
    })
})

describe('SynapticClipBuilder - Integration', () => {
    test('complex composition with fluent API', () => {
        const bridge = createTestBridge()

        const intro = new SynapticClipBuilder(bridge)
            .note('C4', 480)
            .note('E4', 480)
            .note('G4', 480)
            .rest(480)

        const verse = new SynapticClipBuilder(bridge)
            .note('G3', 240)
            .note('C4', 240)
            .note('E4', 240)
            .note('G4', 240)
        ...
        expect(synapseExists(bridge, intro.getBuilder().getExitId(), verse.getBuilder().getEntryId())).toBe(true)
    })
})
