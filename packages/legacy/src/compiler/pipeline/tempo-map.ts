...
export function buildTempoMap(
  ...
  for (const op of sequence.operations) {
    ...
    if (op.kind === 'op' && op.original.kind === 'tempo') {
      ...
      if (envelope) {
        const keyframes = (envelope as import('@symphonyscript/core/types/primitives').TempoEnvelope).keyframes
      ...
      points.push({
        ...
        curve: (curve as import('@symphonyscript/core/types/primitives').TempoCurve) ?? 'linear'
      })
    }
  }
